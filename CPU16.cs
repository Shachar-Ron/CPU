...
namespace Machine
{
    public class CPU16 
    {
        ...
        //7.
        private AndGate m_gAndMemW;
        //8.
        private NotGate nZero;            
        private NotGate nNeg;            
        private AndGate nZ_nNeg;
        private OrGate nORz;

        //9.
        private AndGate m_gAnd2;
        private WireSet zeronum;
        private WireSet onenum;
        ...
        private void ConnectControls()
        {
            ...
            m_gAndD.ConnectInput1(Instruction[D2]);
            m_gAndD.ConnectInput2(Instruction[Type]);
            ...
            m_gOrA.ConnectInput1(Instruction[D1]);

            m_gNotA.ConnectInput(Instruction[Type]);
            m_gOrA.ConnectInput2(m_gNotA.Output);

            m_rA.Load.ConnectInput(m_gOrA.Output);


            //7. connect control to MemoryWrite
            m_gAndMemW = new AndGate();

            m_gAndMemW.ConnectInput1(Instruction[D3]);
            m_gAndMemW.ConnectInput2(Instruction[Type]);

            MemoryWrite.ConnectInput(m_gAndMemW.Output);


            //8. create inputs for jump mux
            nZero = new NotGate();               
            nZero.ConnectInput(m_gALU.Zero);

            nNeg = new NotGate();             
            nNeg.ConnectInput(m_gALU.Negative);

            nZ_nNeg = new AndGate();
            nZ_nNeg.ConnectInput1(nNeg.Output);
            nZ_nNeg.ConnectInput2(nZero.Output);

            nORz = new OrGate();
            nORz.ConnectInput1(m_gALU.Zero);
            nORz.ConnectInput2(m_gALU.Negative);

            //9. connect jump mux (this is the most complicated part)

            //insert all the jumps to the inputs array
            ...
            m_gAnd2.ConnectInput1(Instruction[Type]);
            m_gAnd2.ConnectInput2(m_gJumpMux.Output[0]);
            m_rPC.Load.ConnectInput(m_gAnd2.Output);

            zeronum = new WireSet(1);
            zeronum[0].Value = 0;
            onenum = new WireSet(1);
            onenum[0].Value = 1;


            m_gJumpMux.Inputs[0][0].ConnectInput(zeronum[0]);
            m_gJumpMux.Inputs[1][0].ConnectInput(nZ_nNeg.Output);
            m_gJumpMux.Inputs[2][0].ConnectInput(m_gALU.Zero);
            m_gJumpMux.Inputs[3][0].ConnectInput(nNeg.Output);
            m_gJumpMux.Inputs[4][0].ConnectInput(m_gALU.Negative);
            m_gJumpMux.Inputs[5][0].ConnectInput(nZero.Output);
            m_gJumpMux.Inputs[6][0].ConnectInput(nORz.Output);
            m_gJumpMux.Inputs[7][0].ConnectInput(onenum[0]);
